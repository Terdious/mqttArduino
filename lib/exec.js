...
sails.log.info(`Arduino - Successfully connected to MQTT: OK10`);
    ...
    if (params.deviceType.protocol === 'mqtt') {
        ...
        if (params.deviceType.type === 'mode') {
            sendMqttMsg(params.deviceType, params.state.value);
            sails.log.info(`Test: ${params.state.value}`);
            sails.log.info(`Test2: ${params.state}`);
        }
        else {
...
sails.log.info(`Arduino - Successfully connected to MQTT: ${params.deviceType.deviceState}`);
//sails.log.info(`Arduino - Successfully connected to MQTT: ${params.device.LastValue}`);

sails.log.info(`Arduino - Successfully connected to MQTT: ${params.deviceType.device.LastValue}`);
sails.log.info(`Arduino - Successfully connected to MQTT: ${params.deviceType.LastValue}`);
//sails.log.info(`Arduino - Successfully connected to MQTT: ${params.deviceState.deviceType.Value}`);
};

function sendMqttMsg(deviceType, state.value) {
    ...
    clientProvider.getClient().
        then(
            function (client) {
                client.publish(target, `${state.value}`); // It doesn't like when values are numbers
sails.log.info(`Arduino - Successfully connected to MQTT: ${target} = ${state.value}`);
            }
        );
